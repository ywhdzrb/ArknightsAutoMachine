...
[[nodiscard]] LogLevel parse_log_level(std::string_view str) noexcept
{
    ...
    if (str == "warning" || str == "WARN" || str == "WARNING")
    ...
    switch (config.rotation_policy) {
        ...
        case LogRotationPolicy::Daily: {
            return std::make_shared<spdlog::sinks::daily_file_sink_mt>(
                config.file_path, config.rotation_hour, config.rotation_minute);
        }
...
{
    if (!g_thread_pool) {
        g_thread_pool = std::make_shared<spdlog::details::thread_pool>(kDefaultAsyncQueueSize,
                                                                       kDefaultAsyncThreads);
    }
}
 ...
 * @return spdlog 日志器
 */
[[nodiscard]] std::shared_ptr<spdlog::logger> create_logger(const std::string&  name,
