...
FixedMemoryPool& FixedMemoryPool::operator=(FixedMemoryPool&& other) noexcept
{
    if (this != &other) {
        ...
        chunks_ = other.chunks_;
        total_blocks_.store(other.total_blocks_.load(std::memory_order_relaxed),
                            std::memory_order_relaxed);
        free_blocks_.store(other.free_blocks_.load(std::memory_order_relaxed),
                           std::memory_order_relaxed);
        peak_used_.store(other.peak_used_.load(std::memory_order_relaxed),
                         std::memory_order_relaxed);
        allocation_count_.store(other.allocation_count_.load(std::memory_order_relaxed),
                                std::memory_order_relaxed);
        deallocation_count_.store(other.deallocation_count_.load(std::memory_order_relaxed),
                                  std::memory_order_relaxed);
        growth_count_.store(other.growth_count_.load(std::memory_order_relaxed),
                            std::memory_order_relaxed);
    ...
    while (head != nullptr) {
        ...
        if (free_list_.compare_exchange_weak(
            ...
            if (track_allocations_) {
                ...
                while (
                    used > current_peak
                    && !peak_used_.compare_exchange_weak(
                        current_peak, used, std::memory_order_relaxed, std::memory_order_relaxed)) {
                    // 重试
                }
            }
...
FrameBufferPool::FrameBufferPool(std::size_t buffer_size,
                                 std::size_t initial_count,
                                 std::size_t max_count)
    : buffer_size_(buffer_size),
      pool_(MemoryPoolConfig{.block_size        = buffer_size,
